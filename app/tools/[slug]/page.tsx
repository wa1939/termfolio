...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!tool || tool.slug === "splitter") return { title: "Tool not found" }

  return {
    title: `${tool.title.en} — ${siteConfig.name}`,
    description: tool.summary.en,
    openGraph: {
      title: `${tool.title.en} — ${siteConfig.name}`,
      description: tool.summary.en,
      url: `${siteConfig.siteUrl}${tool.href}`,
    ...
    <div className="min-h-screen bg-term-black text-term-white font-mono flex flex-col">
      ...
      <main className="flex-grow pt-24 pb-12">
        <div className="container mx-auto px-4 max-w-6xl">
          <Link href="/tools" className="mb-8 inline-flex items-center gap-2 text-term-gray transition-colors hover:text-term-cyan">
          ...
          <header className="mb-8 max-w-3xl">
            ...
            <p className="mt-3 text-xs leading-6 text-term-gray">
              No login. No database. Files stay in your browser unless the short-link tool asks TinyURL for a redirect.
            </p>
          </header>

          <EverydayToolClient slug={tool.slug} />
        </div>
      </main>
